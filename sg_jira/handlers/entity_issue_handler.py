...
class EntityIssueHandler(SyncHandler):
    ...
    def accept_jira_event(self, resource_type, resource_id, event):
        ...
        if not shotgun_id or not shotgun_type:
            self._logger.debug(
                "Rejecting event for %s %s. It's not linked to a SG "
    ...
    def _get_jira_issue_and_validate(self, jira_issue_key, shotgun_entity):
        ...
        if not jira_issue:
            ...
            self._logger.warning(
                "Unable to find Jira Issue %s for SG %s (%d)"
        ...
        if shotgun_entity["type"] != jira_shotgun_type or shotgun_entity["id"] != int(
            ...
            self._logger.warning(
                "Rejecting Jira Issue %s. Expected it to be linked to SG "
                "%s (%d) but instead it is linked to SG %s (%s)."
                % (
                    jira_issue_key,
                    shotgun_entity["type"],
                    shotgun_entity["id"],
    ...
    def _create_jira_issue_for_entity(
        ...
        **properties
        ...
        self._logger.info(
            "Creating Jira Issue in Project %s for SG %s '%s' (%d)"
    ...
    def _get_jira_issue_field_sync_value(
        ...
        if not jira_field:
            self._logger.debug(
                "Not syncing SG %s.%s to Jira. No target Jira field "
        ...
        if jira_field not in jira_fields:
            self._logger.warning(
                "Not syncing SG %s.%s to Jira. Target Jira %s %s field "
                "is not editable"
                % (
                    shotgun_entity_type,
                    shotgun_field,
        ...
        if added is not None or removed is not None:
            self._logger.debug(
                "Processing SG list change: added %s, removed %s" % (added, removed)
            )
            ...
            if jira_value is None and shotgun_value:
                ...
                raise InvalidShotgunValue(
                    jira_field,
                    shotgun_value,
                    "Couldn't translate SG value %s to a valid value "
        ...
        if is_array:
                ...
                for removed in shotgun_removed:
                    ...
                    if value in current_value:
                        ...
                        self._logger.debug(
                            ...
                            "Removed SG value was %s" % (value, current_value, removed,)
                        )
            ...
            if current_value:
                for removed in shotgun_removed:
                    ...
                    self._logger.debug(
                        "Current Jira value %s unaffected by SG %s removal."
                    ...
                    if value:
                        ...
                        if added_count > 1:
                            self._logger.warning(
                                ...
                                "field %s. SG added %d values. Using %s "
    ...
    def _get_jira_value_for_shotgun_value(
        ...
        """
        self._logger.debug("Getting Jira value for SG value %s" % shotgun_value)
            ...
            for allowed_value in allowed_values:
                ...
                if isinstance(allowed_value, dict):  # Some kind of Jira Resource
                    ...
                    if allowed_value.lower() == sg_value_name:
                        return allowed_value
            self._logger.warning(
                "SG value '%s' is not in the list of allowed values for "
            ...
            if jira_field in ["assignee", "reporter"]:
                if isinstance(shotgun_value, dict):
                    ...
                    if not email_address:
                        self._logger.warning(
                            "Jira field %s requires an email address but SG "
    ...
    def _sync_shotgun_status_to_jira(self, jira_issue, shotgun_status, comment):
        ...
        if not jira_status:
            self._logger.warning(
                "Unable to find a matching Jira status for SG "
    ...
    def process_jira_event(self, resource_type, resource_id, event):
        ...
        if not shotgun_id.isdigit():
            raise ValueError("Invalid SG id %s, it must be an integer" % shotgun_id)
        shotgun_type = fields.get(self._jira.jira_shotgun_type_field)
        ...
        if not sg_entity:
            ...
            # Entities.
            self._logger.warning(
                "Unable to find SG %s (%s)" % (shotgun_type, shotgun_id)
            )
            return False
        ...
        self._logger.debug(
            "Attempting to sync %s (%s) to SG %s (%d) for event %s"
            % (
                issue_type["name"],
                resource_id,
                sg_entity["type"],
        ...
        for change in changes:
            ...
            try:
                ...
                if shotgun_field:
                    ...
                    self._logger.info(
                        "Syncing Jira %s %s '%s' to SG %s (%d) as value '%s'"
                        % (
                            issue_type["name"],
                            jira_issue["key"],
                            change["field"],
                ...
                self._logger.warning(
                    "Unable to sync Jira %s %s '%s' to SG %s (%d): %s"
                    % (
                        issue_type["name"],
                        jira_issue["key"],
                        change["field"],
    ...
    def _get_shotgun_entity_field_sync_value(
        ...
        if not shotgun_field:
            self._logger.debug(
                "Unable to find a target SG field for Jira field %s" % jira_field_id
            )
            return None, None
    ...
    def _get_shotgun_assignment_from_jira_issue_change(
        ...
        if data_type not in ["multi_entity", "entity"]:
            raise ValueError(
                "%s field type is not valid for SG %s.%s assignments. Expected "
        ...
        if "HumanUser" not in sg_valid_types:
            raise ValueError(
                "SG %s.%s assignment field must accept HumanUser entities "
                "but only accepts %s"
        ...
        if data_type == "multi_entity":
                ...
                if sg_user is not None:
                    for i, current_sg in enumerate(current_sg_assignment):
                        if (
                            ...
                            self._logger.debug(
                                "Removing user %s from SG assignment" % (sg_user)
                            )
            ...
            if to_assignee:
                ...
                for current_sg_user in current_sg_assignment:
                    ...
                    self._logger.debug(
                        "Adding user %s to SG assignment %s"
        ...
        if not sg_user:
            if raise_on_missing_user:
                raise InvalidJiraValue(
                    shotgun_field,
                    jira_user,
                    "Unable to find a SG user with email address %s" % (emailAddress),
                )
            else:
                self._logger.debug(
                    "Unable to find a SG user with email address %s" % (emailAddress)
        ...
        if not sg_user:
            if raise_on_missing_user:
                raise InvalidJiraValue(
                    shotgun_field,
                    jira_user,
                    "Unable to find a SG user with JIRA accountId %s" % (user_id),
                )
            else:
                self._logger.debug(
                    "Unable to find a SG user with JIRA accountId %s" % (user_id)
