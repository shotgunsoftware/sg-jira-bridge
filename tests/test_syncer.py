...
import sg_jira
from sg_jira.constants import (SHOTGUN_JIRA_ID_FIELD, SHOTGUN_JIRA_URL_FIELD,
...
JIRA_EVENT = {
    "changelog": {"id": "123456", "items": [JIRA_SUMMARY_CHANGE]},
    "issue": {
...
class TestJiraSyncer(TestSyncBase):
    ...
    def test_shotgun_event_accept(self, mocked_cur_user, mocked_sg):
        ...
        self.assertFalse(syncer.accept_shotgun_event("Task", 123, event={}))
        # Events without a Project should be rejected
        self.assertFalse(
            syncer.accept_shotgun_event("Task", 123, event={"meta": SG_EVENT_META})
        )
    ...
    def test_jira_event_accept(self, mocked_sg):
        ...
        self.assertFalse(syncer.accept_jira_event("Issue", "FAKED-001", event={}))
        # Check a valid event is accepted
        self.assertTrue(
            syncer.accept_jira_event("Issue", "FAKED-001", event=JIRA_EVENT)
        )

        # Events without a webhookEvent key should be rejected
        event = dict(JIRA_EVENT)
        del event["webhookEvent"]
        self.assertFalse(syncer.accept_jira_event("Issue", "FAKED-001", event=event))
    ...
    def test_project_match(self, mocked_sg):
        ...
        with mock.patch.object(
            syncer.jira, "createmeta", return_value=createmeta
        ) as m_cmeta:  # noqa
    ...
    def test_shotgun_assignee(self, mocked_sg):
        ...
        task = bridge.shotgun.find_one(
            "Task", [["id", "is", 2]], [SHOTGUN_JIRA_ID_FIELD]
        )
    ...
    def _test_jira_assignment(self, mocked_sg, is_jira_cloud):
        ...
        self.assertEqual(
            ...
            bridge.shotgun.find_one(
                sg_entity_type, [["id", "is", sg_entity_id]], ["task_assignees"]
            )["task_assignees"],
        )
    ...
    def test_jira_labels(self, mocked_sg):
        ...
        self.assertEqual(
            [],
            bridge.shotgun.find_one(
                sg_entity_type, [["id", "is", sg_entity_id]], ["tags"]
            )["tags"],
        )
    ...
    def test_jira_status(self, mocked_sg):
        ...
        bridge.shotgun.update(sg_entity_type, sg_entity_id, {"sg_status_list": "rdy"})
        self.assertEqual(
            "rdy",
            bridge.shotgun.find_one(
                sg_entity_type, [["id", "is", sg_entity_id]], ["sg_status_list"]
            )["sg_status_list"],
        )
        jira_event = dict(JIRA_EVENT)
    ...
    def test_unicode(self, mocked_sg):
        ...
        sg_task = {
            "type": sg_entity_type,
            "id": sg_entity_id,
            "content": "%s %s (%d)" % (sg_entity_type, encoded_string, sg_entity_id),
    ...
    def test_shotgun_note(self, mocked_sg):
        ...
        bridge.shotgun.update(
            synced_task["type"], synced_task["id"], {SHOTGUN_SYNC_IN_JIRA_FIELD: True}
        )
    ...
    def test_explicit_sync(self, mocked_sg):
        ...
        bridge.shotgun.update(
            sg_task["type"], sg_task["id"], {SHOTGUN_SYNC_IN_JIRA_FIELD: False}
        )
        ...
        bridge.shotgun.update(
            sg_task["type"], sg_task["id"], {SHOTGUN_SYNC_IN_JIRA_FIELD: True}
        )
    ...
    def test_mismatched_sg_jira_ids(self, mocked_sg):
        ...
        issue = bridge.jira.create_issue(
            {JIRA_ISSUE_SG_TYPE_FIELD: "Task", JIRA_ISSUE_SG_ID_FIELD: 00000}
        )
        self.add_to_sg_mock_db(bridge.shotgun, SG_PROJECTS)
        ...
        self.assertFalse(
            bridge.sync_in_jira("task_issue", "Task", synced_task["id"], event)
        )
